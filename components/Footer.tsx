    ...
    <footer className="bg-black text-chrome py-12 border-t border-steel">
      <div className="container mx-auto px-6">
        <div className="grid md:grid-cols-3 gap-8 mb-12">
          ...
          {/* Links */}
          <div>
            <h4 className="font-bold text-lg mb-4 text-white">{t.footer.navTitle}</h4>
            <ul className="space-y-2 text-sm">
              <li><a href="#services" className="hover:text-electric transition-colors">{t.navbar.solutions}</a></li>
              <li><a href="#portfolio" className="hover:text-electric transition-colors">{t.navbar.portfolio}</a></li>
              <li><a href="#testimonials" className="hover:text-electric transition-colors">{t.navbar.testimonials}</a></li>
            </ul>
          </div>

          {/* Social */}
          <div>
            <h4 className="font-bold text-lg mb-4 text-white">{t.footer.contactTitle}</h4>
            ...
            <a href="https://wa.me/5581985742015" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-sm">
              <Mail size={16} className="text-electric" /> {t.footer.whatsapp}
            </a>
          </div>
        </div>

        <div className="border-t border-steel pt-8 text-center text-xs text-gray-600">
          &copy; {new Date().getFullYear()} VitorDev. {t.footer.rights}
        </div>
      </div>
    </footer>
  );
