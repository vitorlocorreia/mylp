...
const projectAssets: Record<number, { desktopImg: string; mobileImg: string; link: string }> = {
  1: {
...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, index, viewProjectText, desktopText, mobileText }) => {
      ...
      <div className="w-full lg:w-5/12 space-y-6 md:space-y-8 z-10">
        ...
        <div className="flex flex-wrap gap-3 pt-2">
          <div className="flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 bg-anthracite rounded-full text-xs md:text-sm text-chrome border border-steel">
            <Monitor size={14} className="text-white" /> {desktopText}
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 bg-anthracite rounded-full text-xs md:text-sm text-chrome border border-steel">
            <Smartphone size={14} className="text-white" /> {mobileText}
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 bg-anthracite rounded-full text-xs md:text-sm text-chrome border border-steel">
...
const Portfolio: React.FC = () => {
  ...
  const projects: PortfolioItem[] = t.portfolio.items.map((item) => ({
    ...item,
        ...
        <div className="max-w-6xl mx-auto">
          {projects.map((project, index) => (
            <ProjectCard
              ...
              project={project}
              index={index}
              viewProjectText={t.portfolio.viewProject}
              desktopText={t.portfolio.desktop}
              mobileText={t.portfolio.mobile}
            />
          ))}
        </div>
